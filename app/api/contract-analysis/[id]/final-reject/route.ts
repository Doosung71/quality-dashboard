...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$transaction([
    prisma.contractAnalysis.update({ where: { id }, data: { status: "DRAFT", submittedAt: null } }),
    prisma.contractReviewHistory.create({
      data: { analysisId: id, userId: session.user.id, action: "FINAL_REJECT", fromStatus: "REVIEWED", toStatus: "DRAFT", reason },
    }),
  ])
