...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$transaction([
    prisma.contractAnalysis.update({ where: { id }, data: { status: "REVIEWED" } }),
    prisma.contractReviewHistory.create({
      data: { analysisId: id, userId: session.user.id, action: "REVIEW_APPROVE", fromStatus: "DRAFT", toStatus: "REVIEWED" },
    }),
  ])
