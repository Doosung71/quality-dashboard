    ...
    content: [
      {
        ...
        items: [
          ...
          {
            q: "파일 업로드가 실패합니다.",
            a: "세 가지를 확인하세요. ① 형식: PDF 또는 DOCX만 지원합니다. ② 크기: 파일 한 개당 50MB 이하여야 합니다. ③ 파일명: 특수문자(!, @, #, & 등)가 있으면 영문·숫자·한글로 변경 후 재시도하세요.",
          },
          {
            q: "분석 결과 항목이 0건으로 나옵니다.",
            a: "파일이 스캔 이미지 PDF인 경우 텍스트를 읽지 못해 결과가 0건이 됩니다. Word(DOCX) 파일이나 텍스트 레이어가 포함된 PDF로 변환 후 재업로드하세요.",
          },
          {
    ...
    content: [
      {
        ...
        rows: [
          ...
          ["'분석 실패' 오류 메시지", "파일 형식 오류 또는 서버 일시 과부하", "파일 형식(PDF/DOCX) 확인 후 1~2분 뒤 재시도"],
          ...
          ["분석 결과 항목이 0건", "스캔 이미지 PDF (텍스트 없음)", "DOCX 또는 텍스트 포함 PDF로 변환 후 재업로드"],
...
const SECTIONS = {
  PRACTITIONER: [
    ...
    },
    {
      ...
      title: "3. 입찰 검토 AI",
      content: [
        { type: "p", text: "입찰 시방서(PDF/DOCX)를 업로드하면 AI가 독소 조항, 기술 리스크, 누락 항목을 자동 분석합니다. 분석 결과는 Draft 초안으로 생성되며, 반드시 직접 검토 후 사용합니다." },
        { type: "h3", text: "새 입찰 분석 시작" },
        {
          ...
          items: [
            ...
            "시방서 파일(PDF 또는 DOCX, 최대 50MB)을 업로드합니다. 여러 파일 동시 업로드 가능합니다.",
    ...
    },
    {
      ...
      content: [
        { type: "p", text: "로그인 후 메인 대시보드에서 5개 영역의 핵심 KPI를 한눈에 확인합니다. 이상 징후가 있는 영역은 빨간색으로 강조됩니다." },
        {
          ...
          headers: ["카드", "확인 항목"],
          rows: [
            ["시험장 현황", "노후 설비 수, 교체 예정 설비"],
            ["클레임 현황", "미클로징 건수, 평균 처리일, 30일 초과 건"],
            ["협력업체", "C·D 등급 업체 수 (리스크 업체)"],
            ["인사 현황", "면담 예정 인원, 최근 면담일"],
            ["외부정보", "최신 시장·기술 동향 업데이트 수"],
          ],
        },
        {
          type: "steps",
    ...
    },
    {
      ...
      content: [
        ...
        { type: "h3", text: "외부정보" },
        {
          type: "steps",
          items: [
            "'외부정보' 메뉴에서 시장·기술·경쟁사 동향 카드를 확인합니다.",
            "카드를 클릭하면 세부 내용과 출처를 볼 수 있습니다.",
          ],
        },
        {
          type: "checklist",
          ...
          items: [
            ...
            "인사·면담 화면에서 어떤 추가 정보가 있으면 좋을까요?",
            "외부정보 카드의 업데이트 주기와 내용이 유용한가요?",
          ],
        },
      ],
    },
