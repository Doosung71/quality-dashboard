...
import { Search, Plus, ChevronRight, CheckCircle2, XCircle, AlertCircle } from "lucide-react"
...
export default async function SourceInspectionsPage() {
  ...
  const condPass  = inspections.filter(i => i.result === "CONDITIONAL_PASS").length
  const avgDefectRate = inspections.length > 0
    ? inspections.reduce((s, i) => s + (i.defectRate ?? 0), 0) / inspections.length
    : 0
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">출장검사</h1>
          <p className="text-slate-500 text-sm mt-1">Source Inspection — 협력업체 현장 출장검사 결과를 기록합니다.</p>
        </div>
        <Link
          href="/vendors/inspections/new"
        ...
        >
          <Plus className="w-3.5 h-3.5" /> 새 출장검사 등록
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-900">{inspections.length}<span className="text-sm font-normal text-slate-400 ml-1">건</span></p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3">
      ...
      <section className="bg-white rounded-xl border border-slate-200">
        ...
        {inspections.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-sm text-slate-500">등록된 출장검사 결과가 없습니다.</p>
            <Link href="/vendors/inspections/new" className="text-xs text-emerald-500 hover:text-emerald-700 font-medium flex items-center gap-0.5">
              첫 출장검사 결과 등록하기 <ChevronRight className="w-3 h-3" />
            </Link>
          </div>
        ) : (
          <div className="divide-y divide-slate-50">
            {inspections.map(i => {
              ...
              return (
                <Link key={i.id} href={`/vendors/inspections/${i.id}`}
                  className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors group">
                  <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${rc.dot}`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-semibold text-slate-900 group-hover:text-emerald-700">{i.vendorName}</p>
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-slate-400 flex-wrap">
                      <span>{new Date(i.inspectionDate).toLocaleDateString("ko-KR")}</span>
