...
export default function InspectionForm({ vendors: initialVendors, defaultInspector }: { vendors: Vendor[]; defaultInspector: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (attachments.length > 0) {
        await fetch(`/api/source-inspections/${id}`, {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="sm:col-span-2">
          ...
          <select value={form.vendorId} onChange={e => onVendorChange(e.target.value)} className={field} required>
            ...
            <option value={NEW_VENDOR_SENTINEL} className="text-emerald-600 font-semibold">+ 신규업체 등록...</option>
          </select>
        </div>
      ...
      <div className="border-t border-slate-100 pt-4">
        ...
        <AttachmentUploader attachments={attachments} onChange={setAttachments} context="source-inspection" disabled={loading} />
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
          ...
          <form onSubmit={handleNewVendorSubmit} className="space-y-4">
            ...
            <div className="flex gap-2 justify-end pt-2 border-t border-slate-100">
              ...
              <button type="submit" disabled={modalLoading} className="px-5 py-2 text-xs font-semibold bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50">
