...
export function ClaimDetailPage({ claim: initial, canEdit = true, canVerifyLesson = false, userName }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch(`/api/claims/${claim.id}`, {
        method: "PUT", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...editForm, responsibleParty: resolvedParty || null, spg: editForm.spg.trim() || null }),
      });
      ...
      setClaim(prev => ({
        ...
        spg:              editForm.spg.trim() || undefined,
      }));
      setEditing(false);
      router.refresh();
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        {canEdit && (
          <div className="flex items-center gap-2 shrink-0">
            {editing ? (
              ...
              <>
                <button onClick={() => { setEditForm({ title: claim.title, customer: claim.customer, projectKey: claim.projectKey ?? "", priority: claim.priority, assignee: claim.assignee, description: claim.description, targetDate: claim.targetDate ?? "", responsibleParty: claim.responsibleParty ?? "", spg: claim.spg ?? "" }); setCustomParty(""); setEditing(true); }}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl">
      ...
      {/* SPG (제품군) — 자유입력, E2E-1 피드백 #39 */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        <h2 className="text-sm font-bold text-slate-700 mb-3">SPG (제품군)</h2>
        {editing ? (
          <input type="text" placeholder="예: 지중케이블" value={editForm.spg}
            onChange={e => setEditForm(f => ({...f, spg: e.target.value}))}
            className="w-full px-3 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs" />
        ) : (
          <p className="text-sm font-semibold text-slate-800">
            {claim.spg
              ? <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-200">{claim.spg}</span>
              : <span className="text-slate-400 text-xs">미지정</span>
            }
          </p>
        )}
