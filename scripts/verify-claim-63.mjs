// 검증 — E2E-1 #63 Claims 처리이력 시스템/사용자 분리 + 되돌리기 dedup 이식
// 실제 브라우저에서 단계 이동 버튼을 클릭해 배지·dedup DOM을 확인한다.
// 자격증명은 gitignore된 .env.local (WITNESS_VERIFY_*). 테스트 클레임은 정리 삭제.
...
const waitCount = (page, n) =>
  page.waitForFunction((n) => {
    const h2 = [...document.querySelectorAll("h2")].find(h => h.textContent?.includes("처리 이력"))
    return h2?.closest("div")?.querySelectorAll(".group").length === n
...
try {
  ...
  const created = await page.evaluate(async () => {
    const r = await fetch("/api/claims", {
      ...
      body: JSON.stringify({
        title: "클로이검증_삭제예정_63", customer: "검증고객", priority: "Low",
        assignee: "클로이", description: "타임라인 dedup 검증용 (삭제 예정)",
      }),
    })
    return { status: r.status, body: await r.json() }
  })
  testId = created.body?.id
  log(created.status === 201 && testId ? "✅" : "❌", "테스트 클레임 생성(201)", `status ${created.status}`)
  if (!testId) throw new Error("클레임 생성 실패 — 이후 스킵")

  await page.goto(`${BASE}/claims/${testId}`)
  await page.getByRole("heading", { name: "처리 이력" }).waitFor({ timeout: 15000 })
  // 로그인 시 뜨는 모달(공지 "확인했습니다"/온보딩 "닫기")이 클릭을 가로채므로 닫는다.
  ...
  // V1: 생성 직후 — "클레임 접수"는 접두사·kind 없는 사용자 항목 → 배지 0, 항목 1
  await page.waitForFunction(() => document.body.textContent?.includes("클레임 접수"), { timeout: 10000 })
  const v1sys = await sysBadgeCount(page)
  const v1cnt = await timelineCount(page)
  log(v1sys === 0 && v1cnt === 1 ? "✅" : "❌", "V1 생성 직후 시스템 배지 0·항목 1", `sys=${v1sys} cnt=${v1cnt}`)

  // V2: 다음 단계(접수→조사 중) → 시스템 로그 추가 (배지 1, 항목 2)
  await page.getByRole("button", { name: /다음 단계/ }).click()
  await waitCount(page, 2)
  const v2sys = await sysBadgeCount(page)
  const v2cnt = await timelineCount(page)
  log(v2sys === 1 && v2cnt === 2 ? "✅" : "❌", "V2 다음 단계 → 시스템 배지 1·항목 2", `sys=${v2sys} cnt=${v2cnt}`)

  // V3: 이전 단계(되돌리기, 조사 중→접수) → dedup으로 방금 항목 제거 (배지 0, 항목 1)
  ...
  log(v3sys === 0 && v3cnt === 1 ? "✅" : "❌", "V3 되돌리기 dedup → 배지 0·항목 1", `sys=${v3sys} cnt=${v3cnt}`)

  // V4: 수동 메모 추가 → 사용자 항목(배지 없음). 배지 0, 항목 2
  ...
  log(v4sys === 0 && v4cnt === 2 ? "✅" : "❌", "V4 수동 메모 → 배지 그대로 0·항목 2", `sys=${v4sys} cnt=${v4cnt}`)
} catch (e) {
  log("❌", "예외", String(e))
} finally {
  if (testId) {
    const del = await page.evaluate(async (id) =>
      (await fetch(`/api/claims/${id}`, { method: "DELETE" })).status, testId).catch(() => "err")
